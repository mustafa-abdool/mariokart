...
class FP extends JPanel implements KeyListener, MouseListener, MouseMotionListener, Runnable{
	...
	private int real_width = 512;
	private int real_height = 512;
	...
	public void drawScene(Graphics g){
		...
		Graphics2D temp_g = screen.createGraphics();
		temp_g.scale(.5,.5);
		...
		for (int i=0;i<128;i++){
			for (int j=0;j<128;j++){
				if (terrain_grid[i][j].equals("nothing")==false){
					...
					temp_g.drawImage(images.get(type),j*gridratio,i*gridratio,null);
				}
			}
		}

...
class FP extends JPanel implements KeyListener, MouseListener, MouseMotionListener, Runnable{
	...
	public void run(){
		...
		load("outfile0");
		pictureTerrainTest("Koopa_Beach");
	...
	public void pictureTerrainTest( String filename){ //takes in a given map and writes the 2d array to a datafile 
		...
		int pvalue = buffer.getRGB(0,0);
		System.out.println(pvalue);
		pvalue = buffer.getRGB(56,208);
		System.out.println(pvalue);
		...
		for (int i=0;i<1024;i+=gridratio){
			...
			//	System.out.println(i+" "+j);
				...
				else if (pixel==-16738304){//-10465240 || pixel==-10422176 || pixel==-6254496 || pixel==-9940984 || pixel==-10993664){ //dirt
					line+="d";
				}
			...
			//		line+="s";
				...
				else if (pixel==-12558088 || pixel==-13614952 || pixel==-7296776){
					line+="w";
				}

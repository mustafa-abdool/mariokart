...
=======================MOOSEY-O KART FINAL PROJECT - Level Editor====================================


Introduction: This is a level editor used to create maps from the game and modify exist map. It was used to create the text files
that contain the terrain data for each map, by using pixel values. While you can create your own map and save it, I did not focus
on this feature in my game, because the pre-made maps looked better, and it was difficult to get the AI to move perfectly on everymap
without getting stuck in a wall or something.

There are actually several versions of this Level Editor - specific for each map
Because sometimes the pixel values for the maps would coincide and mess things up 

============Controls==============

Arrow Keys - change terrian type
L - large square for terrain type
M - medium sqaure for terrain type
S - small  square for terrain type

Enter - Save a picture of the map and write to a text file which would contain its array data
Left Click - Place terrain type
Right Click - Floodfill terrain type (may crash if area is too large)
...
//===============Setup the MainGame Class (copied from NetTrek)==============
...
//Create Level Editor Class....though its called FP =/
...
class FP extends JPanel implements KeyListener, MouseListener, MouseMotionListener, Runnable{
	...
	private Image picture = new ImageIcon("mushroom map2.PNG").getImage(); //picture of the current map
	
	private String[][] terrain_grid = new String[s_width/gridratio][s_height/gridratio]; //2d arrray represent terrain type
	//you need to divide by grid ratio to get these
	...
	private boolean mb2Down=false; //for the right click
	 ...
	 private HashMap<String,Image> images = new HashMap<String,Image>(); //maps names of maps onto their image 
	 
	 private int imageIndex = 0; //index of selected image
	 private String selectedImage = "barrier blue"; //current selected terrain type
	 private String size = "large"; //size of terraint ype
	 
	 private HashMap<String,Image> maps = new HashMap<String,Image>();	//maps names of maps onto their image 
	 	
	 
	public void loadMaps(){ //Load all the maps and put them into a hasmap
		try{
	...
	public void loadImages(){ //Load all the images and put them into a HashMap
	...
	public FP(MainGame game, int period)throws IOException{ //Set up Jpanel....copied from nettrek
		super();
...
//=========Mouse Events==============
	...
	public void setNewImage(int value){ //Allows you to switch between one image and the next
	
		//This is done by getting the String of the next terrain position in the hashmap
		//Value is a variable which controls what image is currently selected
		//it can either be positive 1 or -1 depending on what key you pressed
	...
	public void run(){
		...
		for (int i=0;i<128;i++){ //fill the grid with nothing to start 
			for (int j=0;j<128;j++){
				terrain_grid[i][j]="n";
			}
		}		
		
		//load FILE that you want to edit 
		
		load("outfile0");
		pictureTerrainTest("Vanilla_Lake");
		...
		while (true){
	    	...
	    	sleepLen = lastUpdate + period - System.currentTimeMillis (); //delay
	...
	public void drawRectangle(int mx,int my, int rx, int ry,Graphics g){ //Simple draw rectagnle function - not used in final version
	...
	public void floodFillCheck(){ //Set up for floodfill Algorithm 
		if (mb2Down==true){
			...
			boolean[][] temp = new boolean[128][128]; ///memorization array
	...
	//Floodfill Recursion
		...
		if (traversed[currentX][currentY]==true){ //if you've already been to square then stop 
			return;
		}
	...
	public void drawScene(Graphics g){ //Draws the 2d array to the screen
		for (int i=0;i<128;i++){
			for (int j=0;j<128;j++){
	...
	public void drawImage(Graphics g){ //Draws a specific Terrain type to the screen 
		...
		if (mbDown==true){ //draw to that location
			...
			int closeX=tx/gridratio; //get the clostest grid square ot where you clicked...int/int gives this
			int closeY = ty/gridratio;

			int decre; //control how big the square will be 
			int incre;
			...
			if (size.equals("medium")){
				...
				for (int i=closeX+decre;i<closeX+incre;i++){
					for (int j=closeY+decre;j<closeY+incre;j++){
						if (j>=0 && i>=0 && j<128 && i<128) //check if its in bounds
	...
	//	System.out.println(buffer.getRGB(16,16));
		...
		for (int i=0;i<1024;i+=gridratio){
			...
			for (int j=0;j<1024;j+=gridratio){
				...
				else if (pixel==-10422176	){  //other grass?
					line+="g";
				}	
	...
	public void pictureTerrainTest( String filename){ //takes in a given map and creates the 2D array for it 
	
		BufferedImage buffer = new BufferedImage(1024,1024,BufferedImage.TYPE_INT_ARGB);
		Graphics2D g2d = buffer.createGraphics();
		g2d.drawImage(new ImageIcon("test.PNG").getImage(),0,0,null);
		...
		for (int i=0;i<1024;i+=gridratio){
			...
			//	System.out.println(i+" "+j);
				...
				if (pixel==-505784 || pixel==-10422176 || pixel ==-460656 || pixel==-9934600 || pixel==-65536 || pixel==-5201808){//barrier 
					line+="b"; 
				}
				...
				else if (pixel==-12558088){ //water
					line+="w";
				}
				
		//		else if (pixel==-16777216){
		//			line+="x"; //darkness...like in Rainbow Road
		//		}
	...
	public void drawGrid(Graphics g){ //draws the grid lines 
	...
	public void paintScreen(){ //render the actual screen
		Graphics g;
		g = this.getGraphics();
	...
	private void startGame(){ //Start Thread
		th = new Thread(this);
		th.start();
	}

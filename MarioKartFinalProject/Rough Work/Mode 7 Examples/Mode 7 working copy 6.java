...
//getAudioClip(getCodeBase(),"laser.wav");	
	...
	public void render(Kart player){ ///Draw everything offscreen first
	   ...
	   // drawBackground(dbg);
		...
		spriteTest(player.getX(),player.getY()+30,players[0].getX(),players[0].getY(),player,dbg);
...
/*		double tx = cx - px;
		...
		double s_length = strip_lengths[459 - round(space_y)];
		double scale = v_width/s_length;
		


		int newX = round((space_x + s_length/2)*scale);
		int newY = 430 - horizon;

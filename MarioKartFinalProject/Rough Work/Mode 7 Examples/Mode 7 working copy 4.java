...
//shells can be shot multiple times if you're on an item square 
...
//				System.out.println("AI Char in Game tenete");
	...
	public void renderFloor3D(Graphics dbg,Kart player){
		...
		double min = strip_heights[strip_heights.length-1]; //get smallest thing
		
		double x_trans = max_width/2;
		double y_trans = max_height;
		
		map_g.translate(x_trans,y_trans);
		...
		for (int i = 0;i<(strip_heights.length-1);i++){
			...
			int ycoord = round(max_height - strip_heights[i]);
			int xcoord = round(x_trans - strip_lengths[i]/2);
			
			
			hstrip = map.getSubimage(xcoord,ycoord,round(strip_lengths[i]),1);
			
			t_xscale = view_width/strip_lengths[i];

			
			sg.scale(t_xscale,1);
			
			sg.drawImage(hstrip,0,0,null);
			
			dbg.drawImage(scaled_strip,500,drawstart,null);
	    ...
	    map_g.translate(-x_trans,-y_trans); //translate back 
	    map_g.rotate(-rotation_angle);
		map_g.translate(-px,-py);

...
class FP extends JPanel implements KeyListener, Runnable{
	...
	public void drawMap(Graphics dbg,Kart player){ //player is the perspective that you see stuff from
	    ...
	    camX = (int)player.xpos + (int)(cam_dist*Math.cos(rotation_angle))*-1;
	    camY = (int)player.ypos + (int)(cam_dist*Math.sin(rotation_angle)); 
	    ...
	    camX+=600; //make this an offset
	    camY+=600;
	    
	    //get subimage of e verything you see 
	    vg.drawImage(map,0,0,view,view,camX-view/2,camY-view/2,camX-view/2+view,camY-view/2+view,null,null);
...
//drawing on a scanline by scanline basis
		...
		for (int i=1;i<strip_no/strip_height;i++){
			...
			scalever_g.scale(xscale-.5,yscale); //scale image		

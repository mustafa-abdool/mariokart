...
class FP extends JPanel implements KeyListener, Runnable{
	...
	public void move(){
		...
		if (keys[38]==true){ //up
			player.xpos+=player.velocity*Math.cos(Math.toRadians(player.angle));
		...
		if (keys[38]==true){ //if up arrow is held
			player.speedUp();
		}
		if (keys[40]==true){
...
//drawing on a scanline by scanline basis
	...
	//		System.out.println("Looping");
			...
			if (i%1==0){ //increase how far you can see in the xdirection 
				dx+=1;
			}
...
class Player{
	...
	public void slowDown(boolean downArrow){
		if (downArrow==true){
			acceleration-=.25;
		}
		else{ //if you aren't holding the down arrow
			if (acceleration>0){
				acceleration-=.25;
			}
		}

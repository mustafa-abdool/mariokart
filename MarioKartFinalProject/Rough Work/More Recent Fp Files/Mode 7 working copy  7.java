...
//getAudioClip(getCodeBase(),"laser.wav");	
	...
	//	spriteTest(player.getX(),player.getY()+36,players[0].getX(),players[0].getY(),player,dbg);
		...
		public void spriteTest(double cx,double cy, double px, double py, Kart player,Graphics g){
		...
		double yscale = v_height/max_height;
		
		double temp_x = cx - px;
		double temp_y = cy - py;

		
		double space_x = temp_x*Math.cos(rotation_angle) + temp_y*Math.sin(rotation_angle);
		double space_y = temp_x*Math.sin(rotation_angle)*-1 + temp_y*Math.cos(rotation_angle);
		
		
		
		int scanline = round(space_y/yscale);
		
		double s_length = strip_lengths[459 - scanline];
		double xscale = v_width/s_length;
		


		int newX = round((space_x + s_length/2)*xscale);
		int newY = v_height - scanline;
		
		

		
		g.setColor(Color.pink);
		g.fillRect(newX-4+v_width,newY-4,8,8); 
		//

...
class FP extends JPanel implements KeyListener, Runnable{
	...
	public void drawMap(Graphics dbg,Kart player){ //player is the perspective that you see stuff from
		...
		double rotation_angle = Math.toRadians(player.angle - 90);
		...
		System.out.println(player.angle+180);
	    ...
	    for (int screen_y = 40; screen_y<s_height;screen_y++){
			...
			int map_x_end = map_x + line_dx*s_width;
			int map_y_end = map_y + line_dy*s_swidth;
			
			map_g.drawline(map_x,map_y,map_x_end,map_y_end);

...
class FP extends JPanel implements KeyListener, Runnable{
	...
	private int[][] item_locations;
	
	
	
	public void checkItems(Graphics2D g){
		g.setColor(Color.red);
		
		for (int[] item_data: item_locations){
			if (item_data[3]==0){
				int x = item_data[1];
				int y = item_data[2];
				
			//	System.out.println(x+" "+y);
				g.fillRect(x,y,16,16); //item box width/length is 16x16
			}
		}
	}
	...
	public void loadItems(){
		...
		//1 is avaiable (item can be accessed) 
		//0 is not
		
		try{
			Scanner infile = new Scanner(new File("Item Box Locations.txt"));
			int no = Integer.parseInt(infile.nextLine());
			item_locations = new int[no][4];
			for (int i=0;i<no;i++){
				String line = infile.nextLine();
				System.out.println(line);
				
				String[] data = line.split(" ");
				item_locations[i][0] = Integer.parseInt(data[0]); //upright or inverted
				item_locations[i][1] = Integer.parseInt(data[1]);
				item_locations[i][2] = Integer.parseInt(data[2]);
				item_locations[i][3] = 0; //whether the item box is avaiable or not 
			}

...
class FP extends JPanel implements KeyListener, Runnable{
	...
	public void drawMap(Graphics dbg,Kart player){ //player is the perspective that you see stuff from
		...
		cx = px;//+player_dist*Math.cos(Math.toRadians(player.angle)); //get cx, cy ==> I'm not sure if this is right 
		cy = py;//+player_dist*Math.sin(Math.toRadians(player.angle))*-1;
	    ...
	    for (int screen_y = 40; screen_y<s_height;screen_y++){
	    	line_dist = (cam_height*scale_y)/(screen_y+horizon);
	    //	System.out.println(line_dist);
	    	
	    	line_scale = line_dist/scale_x;
	    //	System.out.println(line_scale);
	    	
	    	line_dx = -sin_precalc*line_scale;
	    	line_dy = cos_precalc*line_scale;
	    	
	    	//starting pos
	    	map_x = cx+line_dist*cos_precalc-(double)s_width/2*line_dx; //whats the purpose of the 512? It doesn't seem to change anything...? O_o
	    ...
	    //		System.out.println(screenx+" "+screen_y);

...
class FP extends JPanel implements KeyListener, Runnable{
	...
	public void drawMap(Graphics dbg,Kart player){ //player is the perspective that you see stuff from
		...
		BufferedImage screen = new BufferedImage(900,900,BufferedImage.TYPE_INT_ARGB); //holds a copy of the screen
		...
		double cam_dist = 35; //dist of camera to screen
		double player_dist = 45; //dist from camera to play
		...
		cx = px+player_dist*Math.cos(Math.toRadians(player.angle))*-1; //get cx, cy
		cy = py+player_dist*Math.sin(Math.toRadians(player.angle));
		
		map_g.drawImage(norm,0,0,null);
		
	    BufferedImage viewrange = new BufferedImage(view+100,view+100,BufferedImage.TYPE_INT_ARGB);
	    Graphics2D vg = viewrange.createGraphics();
	    
	    viewrange = map.getSubimage(round(cx)-view/2,round(cy)-view,view,view);
	    

		
		
//		map_g.setColor(Color.black);
//		map_g.fillRect(0,0,1024,1024);
		
		sg.drawImage(viewrange,300,300,null);
		//cx, cy, is 350 400
		
		int newX = 350;
		double newY = 400;
		
		int scanline_no = view-round(cam_dist);
		double inc = scanline_no/(double)100; //100 is really the s_width or w/e
		
		int ypos = s_height; //y position to draw strip 
		
		double x = 0;
			
		for (double viewY = newY-cam_dist; viewY>newY-100;viewY-=inc){
			
			BufferedImage strip;
			BufferedImage scaled_v;
						
			double delta_y = newY-viewY;
			double lateral = Math.tan(Math.toRadians(FOV/2))*delta_y;
			int strip_width = round(lateral*2);
			
		//	System.out.println(strip_width);
			
			
			strip = new BufferedImage(strip_width,1,BufferedImage.TYPE_INT_ARGB);
			strip = screen.getSubimage(newX-round((lateral/2)),round(viewY),strip_width,1);
			
			double xscale = (double)(s_width)/strip_width;
			
			scaled_v = new BufferedImage(s_width,1,BufferedImage.TYPE_INT_ARGB);
			Graphics2D temp = scaled_v.createGraphics();
			temp.scale(xscale+x,1);
			
			temp.drawImage(strip,0,0,null);
			
			dbg.drawImage(scaled_v,0,ypos,null);
			
			ypos--;
			x-=.002; //some variable thats helps things look right O_o

...
//The red shell was not used in the final version of the game b/c I could not get it to home in on a player correctly
//but here's the class anyways!
...
public class RedShell {
	...
	private double x_leftpoint; //for movement...turning corners 
	...
	public double dist(double x1, double y1, double x2, double y2){ //distance function 
	...
	public boolean isClose2(){ //returns true if the red shell is close to the target 
	...
	public boolean isClose(){
		...
		if (distance<50){
			...
			double delta_y = ypos - target.getY(); //adjust the angle if close 
	...
	public void checkCollision(){ //check red shell collisions with its player target
    ...
    public RedShell(double x, double y, double angle, char[][] terrain, char[][] ai_terrain, FP.Kart target,double ai_stat ) {
 	...
 	public boolean allBorderCheck(){ //border collisoin
    ...
    public void move(){ //Move shell...see AI movement for details
    ...
    public boolean isAvailable(){ //true if you can use it...false if otherwise
 	...
 	public void moveAlong(){ //moves the shell...very slowly in order to check for collisions
	...
	public boolean AIoffRoadCheck(){ //check if the shell is on the dirt 
	...
	public boolean offCourseCheck(){ //check if shell is on snow 
	...
	public boolean AIpointDirtCheck(double px,double py){ //check if shell is on dirt, snow, or offcourse 
		int newx = (int)px;
		int newy = (int)py;
